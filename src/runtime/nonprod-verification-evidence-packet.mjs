...
const REQUIRED_PACKET_FIELDS = Object.freeze([
  'planId','approvalRef','targetRef','migrationArtifact','rlsPolicy','regressionEvidence','integrityEvidence','releaseManifest','rollbackPlan','independentReview',
]);
...
function syntheticRef(value, name) { const ref = requiredString(value, name); if (!/^synthetic:[A-Za-z0-9._:-]+$/.test(ref)) throw new Error(`${name}_MUST_BE_SYNTHETIC_REF`); return ref; }
...
export function buildNonProductionEvidencePacket({ planId, checkpoint = 'P10.180', approvalRef = null, targetRef = null, migrationArtifact = null, rlsPolicy = null, regressionEvidence = null, integrityEvidence = null, releaseManifest = null, rollbackPlan = null, independentReview = null } = {}) {
  ...
  const packet = {
    ...
    approvalRef: approvalRef === null ? null : syntheticRef(approvalRef, 'APPROVAL_REF'),
    targetRef: targetRef === null ? null : syntheticRef(targetRef, 'TARGET_REF'),
    migrationArtifact: migrationArtifact === null ? null : structuredClone(migrationArtifact),
    rlsPolicy: rlsPolicy === null ? null : structuredClone(rlsPolicy),
    regressionEvidence: regressionEvidence === null ? null : structuredClone(regressionEvidence),
    integrityEvidence: integrityEvidence === null ? null : structuredClone(integrityEvidence),
    releaseManifest: releaseManifest === null ? null : structuredClone(releaseManifest),
    rollbackPlan: rollbackPlan === null ? null : structuredClone(rollbackPlan),
    independentReview: independentReview === null ? null : structuredClone(independentReview),
  };
...
export function assertNonProductionEvidencePacket(packet) {
  ...
  if (packet.migrationArtifact?.syntheticOnly !== true || !packet.migrationArtifact?.sha256) throw new Error('PACKET_MIGRATION_ARTIFACT_INVALID');
  if (packet.rlsPolicy?.syntheticOnly !== true || !packet.rlsPolicy?.sha256) throw new Error('PACKET_RLS_POLICY_INVALID');
  for (const [name, evidence] of [['regressionEvidence',packet.regressionEvidence],['integrityEvidence',packet.integrityEvidence],['releaseManifest',packet.releaseManifest]]) {
    if (!evidence?.syntheticOnly === true) throw new Error(`PACKET_${name.toUpperCase()}_NOT_SYNTHETIC`);
  }
  ...
  if (packet.rollbackPlan?.syntheticOnly !== true || !packet.rollbackPlan?.sha256) throw new Error('PACKET_ROLLBACK_PLAN_INVALID');
  if (packet.independentReview?.syntheticOnly !== true || packet.independentReview?.reviewerRef === undefined || packet.independentReview?.role === undefined) throw new Error('PACKET_INDEPENDENT_REVIEW_INVALID');
