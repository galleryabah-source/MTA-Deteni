...
export function createRuntimeHandoff(input: { executionId: string; fromMode: RuntimeMode; toMode: RuntimeMode; queuePending: boolean; }): RuntimeHandoff {
  assertIdentity(input.executionId);
  if (input.fromMode === input.toMode) throw new Error("Runtime handoff requires a mode transition.");
  const reconciliationRequired = input.queuePending || input.fromMode !== "CLOUD";
  return Object.freeze({ fromMode: input.fromMode, toMode: input.toMode, executionId: input.executionId, queuePending: input.queuePending, reconciliationRequired, authorizationRequired: true, certificationBound: true, syntheticOnly: true });
}

export function assertRuntimeHandoffSafety(handoff: RuntimeHandoff): void {
  if (!handoff.executionId.trim() || !handoff.syntheticOnly || !handoff.certificationBound) throw new Error("Runtime handoff is not certified.");
