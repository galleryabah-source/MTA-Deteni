...
export function assertLocalRuntimeRecoveryOperationalAuditPublicationCertification(input: LocalRuntimeRecoveryOperationalAuditPublicationCertification, envelope: LocalRuntimeRecoveryOperationalAuditPublicationEnvelope): void {
  if (!input.publicationCertificationId.trim() || !input.certified || !input.syntheticOnly || input.externalPublicationPerformed || input.replayDisposition === "CONFLICT") throw new Error("Operational audit publication certification must be certified, non-conflicted, unpublished and synthetic-only.");
