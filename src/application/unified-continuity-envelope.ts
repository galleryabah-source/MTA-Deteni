...
export function createUnifiedContinuityEnvelope(input: {
  ...
  if (!input.reconciliation.complete || input.reconciliation.admittedCount !== input.reconciliation.reconciledCount) throw new Error("Unified continuity envelope requires complete reconciliation proof.");
  return Object.freeze({ envelopeId: input.envelopeId, certificationId: input.certification.certificationId, sessionId: input.session.sessionId, executionId: input.session.executionId, deviceId: input.session.deviceId, installationId: input.session.installationId, networkScopeId: input.session.networkScopeId, journeyId: input.certification.journeyId, admittedCount: input.reconciliation.admittedCount, reconciledCount: input.reconciliation.reconciledCount, reconciliationReceiptIds: Object.freeze([...input.reconciliation.receiptIds]), handoffKey: `${input.handoff.fromMode}->${input.handoff.toMode}:${input.handoff.executionId}`, backupDecision: input.backup.decision, certified: true, syntheticOnly: true });
}

export function assertUnifiedContinuityEnvelope(input: UnifiedContinuityEnvelope): void {
  const values = [input.envelopeId, input.certificationId, input.sessionId, input.executionId, input.deviceId, input.installationId, input.networkScopeId, input.journeyId, input.handoffKey];
  ...
  if (input.certificationId !== input.certification.certificationId) throw new Error("Unified continuity certification identity mismatch.");
  if (input.journeyId !== input.certification.journeyId || input.certification.runtimeDecision !== "READY" || input.backupDecision !== "READY") throw new Error("Unified continuity readiness binding mismatch.");
  if (input.admittedCount !== input.reconciledCount || input.reconciliationReceiptIds.length !== input.reconciledCount) throw new Error("Unified continuity command/receipt cardinality mismatch.");
}
