...
export function createLocalRuntimeRecoveryOperationalAuditProjection(input: {
  projectionId: string;
  evidenceCertification: LocalRuntimeRecoveryFinalClosureAuditEvidenceCertification;
  evidence: {
    evidenceId: string;
    auditCertificationId: string;
    auditRecordId: string;
...
}): LocalRuntimeRecoveryOperationalAuditProjection {
  ...
  if (!c.certified || !c.syntheticOnly || c.replayDisposition === "CONFLICT") throw new Error("Operational audit projection requires certified, non-conflicted synthetic evidence.");
...
export function assertLocalRuntimeRecoveryOperationalAuditProjection(input: LocalRuntimeRecoveryOperationalAuditProjection, evidenceCertification: LocalRuntimeRecoveryFinalClosureAuditEvidenceCertification): void {
  ...
  if (!evidenceCertification.certified || !evidenceCertification.syntheticOnly || evidenceCertification.replayDisposition === "CONFLICT") throw new Error("Operational audit projection source certification is not admissible.");
