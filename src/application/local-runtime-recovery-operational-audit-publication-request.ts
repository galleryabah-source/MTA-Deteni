...
export function createLocalRuntimeRecoveryOperationalAuditPublicationRequest(input: { requestId: string; certification: LocalRuntimeRecoveryOperationalAuditPublicationCertification; envelope: LocalRuntimeRecoveryOperationalAuditPublicationEnvelope }): LocalRuntimeRecoveryOperationalAuditPublicationRequest {
  ...
  if (!c.certified || !c.syntheticOnly || c.externalPublicationPerformed) throw new Error("Publication request requires certified, unpublished synthetic certification.");
  ...
  if (c.publicationId !== e.publicationId || c.certificationId !== e.certificationId || c.decisionFingerprint !== e.decisionFingerprint) throw new Error("Publication request source identity drift.");
  return Object.freeze({ requestId: input.requestId, publicationCertificationId: c.publicationCertificationId, publicationId: e.publicationId, evidenceCertificationId: e.evidenceCertificationId, evidenceId: e.evidenceId, auditCertificationId: e.auditCertificationId, auditRecordId: e.auditRecordId, closureCertificationId: e.closureCertificationId, closureEvidenceId: e.closureEvidenceId, continuityCertificationId: e.continuityCertificationId, receiptId: e.receiptId, closureId: e.closureId, executionId: e.executionId, dispatchId: e.dispatchId, acknowledgementId: e.acknowledgementId, decisionFingerprint: e.decisionFingerprint, requestState: "ADMITTED", externalTransportRequested: false, syntheticOnly: true });
}
...
export function assertLocalRuntimeRecoveryOperationalAuditPublicationRequest(input: LocalRuntimeRecoveryOperationalAuditPublicationRequest, certification: LocalRuntimeRecoveryOperationalAuditPublicationCertification, envelope: LocalRuntimeRecoveryOperationalAuditPublicationEnvelope): void {
  ...
  if (!certification.certified || certification.externalPublicationPerformed || !certification.syntheticOnly) throw new Error("Publication certification is not admissible.");
