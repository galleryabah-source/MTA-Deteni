...
export function openOperationalSession(input: { sessionId: string; context: RuntimeExecutionContext; deviceId: string; installationId: string }): OperationalSession {
...
export function assertSessionScope(session: OperationalSession, context: RuntimeExecutionContext, deviceId: string, installationId: string): void {
  ...
  if (session.runtimeMode !== context.runtimeMode) throw new Error("Operational session runtime mode drift detected.");
...
export function admitLocalCommand(input: { session: OperationalSession; context: RuntimeExecutionContext; deviceId: string; installationId: string; command: OfflineCommand }): CommandAdmission {
  ...
  if (input.session.runtimeMode !== "LAN" && input.session.runtimeMode !== "LOCAL") throw new Error("Local command admission requires LAN or LOCAL runtime.");
  if (input.session.state !== "ACTIVE") throw new Error("Local command admission requires an active operational session.");
  if (!input.command.commandId.trim() || !input.command.aggregateId.trim() || !input.command.commandType.trim() || !input.command.payloadHash.trim() || !input.command.idempotencyKey.trim() || !input.command.createdAt.trim()) throw new Error("Local command identity is required.");
...
export function assessSessionClose(input: { session: OperationalSession; context: RuntimeExecutionContext; deviceId: string; installationId: string; queue: readonly OfflineCommand[]; reconciliation?: ReconciliationDecision; runtime: RuntimeContinuityAssessment; backup: BackupContinuityAssessment; continuity: ContinuityCertification }): SessionCloseEvidence {
  ...
  if (input.runtime.executionId !== input.session.executionId || input.runtime.decision !== "READY" || input.runtime.queueState !== "SYNCED") throw new Error("Session close requires reconciled runtime continuity.");
...
export function closeOperationalSession(session: OperationalSession, evidence: SessionCloseEvidence): OperationalSession {
  if (session.state !== "ACTIVE" && session.state !== "RECONCILIATION_REQUIRED") throw new Error("Only an open operational session can be closed.");
  if (evidence.sessionId !== session.sessionId || evidence.executionId !== session.executionId || evidence.deviceId !== session.deviceId || evidence.installationId !== session.installationId || evidence.networkScopeId !== session.networkScopeId || !evidence.clean || !evidence.syntheticOnly) throw new Error("Clean session-close evidence does not match session.");
