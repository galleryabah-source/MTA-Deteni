...
export type ApiCommandRequest<TPayload> = Readonly<{ commandId: string; permission: string; fingerprint: string; payload: TPayload; actor: ActorContext }>;
export type ApiCommandResponse<TResult> = Readonly<{ ok: true; commandId: string; correlationId: string; aggregateId: string; result: TResult }>;
export type ApiErrorResponse = Readonly<{ ok: false; code: "INVALID_REQUEST" | "FORBIDDEN" | "CONFLICT" | "INTERNAL_ERROR" }>;
