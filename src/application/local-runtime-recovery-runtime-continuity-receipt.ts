...
export function assertLocalRuntimeRecoveryRuntimeContinuityReceipt(receipt: LocalRuntimeRecoveryRuntimeContinuityReceipt, continuity: ContinuityCertification, completionProof: LocalRuntimeRecoveryExecutionCompletionProof, acknowledgementCertification: LocalRuntimeRecoveryExecutionAcknowledgementCertification): void {
  ...
  if (!completionProof.completed || !completionProof.syntheticOnly) throw new Error("Recovery execution completion proof must be complete and synthetic-only.");
  if (!acknowledgementCertification.certified || !acknowledgementCertification.admitted || !acknowledgementCertification.syntheticOnly) throw new Error("Runtime continuity receipt requires certified acknowledgement.");
  if (completionProof.acknowledgementCertificationId !== acknowledgementCertification.certificationId || completionProof.certificationId !== acknowledgementCertification.integratedCertificationId || completionProof.executionId !== acknowledgementCertification.executionId || completionProof.dispatchId !== acknowledgementCertification.dispatchId || completionProof.acknowledgementId !== acknowledgementCertification.acknowledgementId || completionProof.decisionFingerprint !== acknowledgementCertification.decisionFingerprint) throw new Error("Runtime continuity receipt completion chain drift.");
