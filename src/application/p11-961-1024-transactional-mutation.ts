...
export type TransactionalMutationDeps<T> = Readonly<{
  ...
  mutate: (actor: ActorContext) => Promise<MutationEnvelope<T>>;
}>;
...
export class TransactionalMutationService<T> {
  constructor(private readonly deps: TransactionalMutationDeps<T>) {}

  async execute(command: MutationCommand): Promise<MutationEnvelope<T>> {
    if (!command.actor.actorId.trim() || !command.actor.correlationId.trim()) {
    ...
    if (!(await this.deps.authorization.authorize(command.actor, command.permission))) {
    ...
    return this.deps.transaction.run(async () => {
      ...
      const envelope = await this.deps.mutate(command.actor);
      ...
      // Implementations MUST bind these writes and idempotency completion to the same transaction.
