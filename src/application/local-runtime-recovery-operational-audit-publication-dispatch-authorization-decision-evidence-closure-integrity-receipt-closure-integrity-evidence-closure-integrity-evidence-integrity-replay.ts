...
export function resetLocalRuntimeRecoveryOperationalAuditPublicationDispatchAuthorizationDecisionEvidenceClosureIntegrityReceiptClosureIntegrityEvidenceClosureIntegrityEvidenceIntegrityReplayRegistry(): void {
  registry.clear();
}

export function replayLocalRuntimeRecoveryOperationalAuditPublicationDispatchAuthorizationDecisionEvidenceClosureIntegrityReceiptClosureIntegrityEvidenceClosureIntegrityEvidenceIntegrity(input: {
  integrity: LocalRuntimeRecoveryOperationalAuditPublicationDispatchAuthorizationDecisionEvidenceClosureIntegrityReceiptClosureIntegrityEvidenceClosureIntegrityEvidenceIntegrity;
  ...
  if (
    input.integrity.evidenceId !== input.evidence.evidenceId ||
    input.integrity.integrityCertificationId !== input.evidence.integrityCertificationId ||
    input.integrity.evidenceCertificationId !== input.evidence.evidenceCertificationId ||
    input.integrity.closureCertificationId !== input.evidence.closureCertificationId ||
    input.integrity.closureId !== input.evidence.closureId ||
    input.integrity.receiptId !== input.evidence.receiptId ||
    input.integrity.decisionCertificationId !== input.evidence.decisionCertificationId ||
    input.integrity.decisionId !== input.evidence.decisionId ||
    input.integrity.decisionFingerprint !== input.evidence.decisionFingerprint
  ) {
    throw new Error("Terminal evidence integrity evidence identity drift.");
  }

  const key = `${input.integrity.integrityId}:${input.evidence.integrityCertificationId}:${input.evidence.evidenceId}`;
  const existing = registry.get(key);
  if (existing === undefined) {
    registry.set(key, input.integrity.decisionFingerprint);
    return "ADMIT";
  }
