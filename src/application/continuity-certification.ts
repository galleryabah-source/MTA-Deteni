...
export function certifyContinuity(input: {
  ...
  for (const value of [input.certificationId, input.sessionId, input.deviceId, input.installationId, input.context.executionId, input.context.networkScopeId, input.context.certificationJourneyId]) if (!value.trim()) throw new Error("Continuity certification identity is required.");
  ...
  if (input.runtime.executionId !== input.context.executionId) throw new Error("Continuity runtime execution drift detected.");
  if (input.backup.sourceDeviceId !== input.deviceId || input.backup.sourceInstallationId !== input.installationId || input.backup.sourceRuntime !== input.context.runtimeMode) throw new Error("Continuity backup source identity drift detected.");
  ...
  return Object.freeze({ certificationId: input.certificationId, journeyId: input.context.certificationJourneyId, executionId: input.context.executionId, sessionId: input.sessionId, deviceId: input.deviceId, installationId: input.installationId, networkScopeId: input.context.networkScopeId, lifecycleJourneyId: input.lifecycle.journeyId, recoveryJourneyId: input.recovery.journeyId, runtimeDecision: input.runtime.decision, backupDecision: input.backup.decision, projectionVersion: lifecycleVersion, lifecycleVersion, certified: true, syntheticOnly: true });
}

export function assertContinuityCertification(input: ContinuityCertification): void {
  for (const value of [input.certificationId, input.journeyId, input.executionId, input.sessionId, input.deviceId, input.installationId, input.networkScopeId]) if (!value.trim()) throw new Error("Continuity certification identity is invalid.");
