...
import { assertRecoveryRetryKeyBinding } from "./recovery-retry.js";
...
export function certifyRecoveryJourney(input: {
  ...
  if (committed) {
    ...
    const fingerprint = input.retries[0]?.sourceFingerprint;
    if (!fingerprint) throw new Error("Committed recovery retry fingerprint is required.");
    input.retries.forEach((retry) => assertRecoveryRetryKeyBinding(retry, evidence.commandId, fingerprint));
    if (input.retries.some((retry) => retry.sourceFingerprint !== fingerprint)) throw new Error("Recovery retry fingerprint drift detected.");
  }
