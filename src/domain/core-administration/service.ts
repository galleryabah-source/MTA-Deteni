...
export class CoreAdministrationService {
  ...
  async register(input: {
    ...
    if (await this.deps.repository.get(input.id)) {
      throw new DomainError("CONFLICT", "Detainee already exists.");
    }
  ...
  async changeStatus(input: {
    ...
    if (!ALLOWED_STATUS_TRANSITIONS[current.status].includes(input.to)) {
      throw new DomainError("INVALID_STATE", `Invalid detainee status transition: ${current.status} → ${input.to}.`);
    }
    const updated: Detainee = {
      ...current,
      status: input.to,
    ...
    await this.deps.audit.append({
      eventType: "DETAINEE_STATUS_CHANGED",
      aggregateType: "DETAINEE",
      aggregateId: updated.id,
      actorId: input.actor.actorId,
      correlationId: input.actor.correlationId,
      payloadHash: `${current.status}:${input.to}:${updated.version}`,
    });
    return updated;
  }

  private assertActor(actor: ActorContext): void {
    if (!this.deps.canManage(actor)) {
      throw new DomainError("FORBIDDEN_SCOPE", "Actor is not authorized for core administration.");
    }
  }
}
