...
export type PlacementDeps = Readonly<{
...
export class PlacementService {
  ...
  async assign(input: {
    ...
    for (const value of [input.detaineeId, input.blockId, input.roomId, input.bedId]) {
      if (!value.trim()) throw new DomainError("VALIDATION_FAILED", "Placement identifiers are required.");
    }
    if (await this.deps.repository.getBedOccupant(input.bedId)) {
