...
export type MovementEvent = Readonly<{
  id: string;
  detaineeId: string;
  type: "IN" | "OUT" | "TRANSFER" | "TEMPORARY_EXIT_DEPARTURE" | "TEMPORARY_EXIT_RETURN";
  fromPlacementRef?: string;
  toPlacementRef?: string;
  occurredAt: string;
  actorId: string;
...
export type MovementRepository = {
  append(event: MovementEvent): Promise<void>;
  listSince(detaineeId: string, since: string): Promise<readonly MovementEvent[]>;
};

export type HeadcountProjection = Readonly<{
  occupied: number;
  outside: number;
  unknown: number;
  asOf: string;
...
export type MovementDeps = Readonly<{
...
export class MovementService {
  ...
  async record(input: Omit<MovementEvent, "occurredAt" | "actorId" | "correlationId"> & { actor: ActorContext; occurredAt?: string }): Promise<MovementEvent> {
    ...
    if (input.type === "TRANSFER" && (!input.fromPlacementRef || !input.toPlacementRef)) {
    ...
    const event: MovementEvent = {
      ...
      type: input.type,
      ...(input.fromPlacementRef ? { fromPlacementRef: input.fromPlacementRef } : {}),
      ...(input.toPlacementRef ? { toPlacementRef: input.toPlacementRef } : {}),
  ...
  async reconcileHeadcount(input: {
    detaineeIds: readonly string[];
    since: string;
    actor: ActorContext;
    ...
    let outside = 0;
    let unknown = 0;
    for (const id of input.detaineeIds) {
      const event = latest.get(id);
      if (!event) unknown++;
      else if (event.type === "OUT" || event.type === "TEMPORARY_EXIT_DEPARTURE") outside++;
    }
