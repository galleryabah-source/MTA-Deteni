...
if (evidence.checkpoint !== 'P10.22') failures.push('WRONG_CHECKPOINT');
...
if (!evidence.testOutputSha256) failures.push('MISSING_OUTPUT_HASH');
if (evidence.testOutputSha256 !== createHash('sha256').update(output).digest('hex')) failures.push('OUTPUT_HASH_MISMATCH');

const result = {
  schemaVersion: 'p10.23.v1',
  checkpoint: 'P10.23',
