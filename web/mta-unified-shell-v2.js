...
const BASE=['/desktop-shell-v2.css?v=7','/responsive-v11.css?v=16','/mta-ui-polish-v1.css?v=4'];
const SCRIPTS=[];
const DB_DISCONNECTED='DB_DISCONNECTED';
const MIGRATION_FREEZE='MIGRATION_FREEZE';
function appendAuditToState(state,action,type,id,result='SUCCESS',correlationId){state.audit=Array.isArray(state.audit)?state.audit:[];state.audit.unshift({id:'AUD-'+crypto.randomUUID().slice(0,8).toUpperCase(),action,resourceType:type,resourceId:id||'',result,occurredAt:new Date().toISOString(),actor:'DEMO-OPERATOR',requestId:'REQ-'+crypto.randomUUID().slice(0,8).toUpperCase(),correlationId:correlationId||'COR-'+crypto.randomUUID().slice(0,8).toUpperCase(),policyVersion:'AUTHZ-1.0'})}
function assignPlacementCommand(state,options={}){const d=state||read();d.detainees=Array.isArray(d.detainees)?d.detainees:[];d.rooms=Array.isArray(d.rooms)?d.rooms:[];d.placements=Array.isArray(d.placements)?d.placements:[];const detaineeId=String(options.detaineeId||''),roomId=String(options.roomId||''),det=d.detainees.find(x=>x.id===detaineeId),target=d.rooms.find(x=>x.id===roomId),requestKey=String(options.requestKey||('PLACEMENT:'+detaineeId+':'+roomId));if(!det)return{ok:false,code:'DETAINEE_NOT_FOUND'};if(det.status!=='AKTIF')return{ok:false,code:'DETAINEE_INACTIVE'};if(!target)return{ok:false,code:'ROOM_NOT_FOUND'};if(target.status!=='ACTIVE')return{ok:false,code:'ROOM_INACTIVE'};const existing=d.placements.find(x=>x.requestKey===requestKey);if(existing)return{ok:true,code:'PLACEMENT_ALREADY_PROCESSED',placement:existing,correlationId:options.correlationId||null};const current=d.placements.filter(x=>x.detaineeId===detaineeId).sort((x,y)=>String(y.since||'').localeCompare(String(x.since||'')))[0]||null;if(current?.roomId===target.id){appendAuditToState(d,'PLACEMENT_ASSIGN_BLOCKED','PLACEMENT',detaineeId,'DENIED',options.correlationId);return{ok:false,code:'SAME_ROOM'}}const occupancy=d.detainees.filter(x=>x.status==='AKTIF'&&x.id!==detaineeId).filter(x=>{const p=d.placements.filter(y=>y.detaineeId===x.id).sort((p1,p2)=>String(p2.since||'').localeCompare(String(p1.since||'')))[0];return p&&(p.roomId===target.id||(p.block===target.block&&p.room===target.room))}).length;const capacity=Number(target.capacity)||0;if(capacity>0&&occupancy>=capacity){appendAuditToState(d,'PLACEMENT_CAPACITY_BLOCKED','PLACEMENT',target.id,'DENIED',options.correlationId);return{ok:false,code:'ROOM_FULL'}}const correlationId=options.correlationId||'COR-'+crypto.randomUUID().slice(0,8).toUpperCase(),placement={id:options.id||'PLC-'+crypto.randomUUID().slice(0,10).toUpperCase(),detaineeId,roomId:target.id,blockId:target.blockId,block:target.block,room:target.room,since:options.since||new Date().toISOString(),source:options.source||'MASTER_ROOM',requestKey,movementId:options.movementId||null,correlationId};d.placements.unshift(placement);det.placement=target.block+' / '+target.room;d.lastMutation={key:requestKey,action:options.action||'PLACEMENT_ASSIGN',completedAt:new Date().toISOString()};appendAuditToState(d,'PLACEMENT_ASSIGN','PLACEMENT',placement.id,'SUCCESS',correlationId);return{ok:true,code:'PLACEMENT_ASSIGNED',placement,correlationId}}
function advanceLeaveCommand(state,id){const d=state||read(),leave=(d.leaves||[]).find(x=>x.id===id);if(!leave)return{ok:false,code:'LEAVE_NOT_FOUND'};const next={DRAFT:'SUBMITTED',SUBMITTED:'APPROVED',APPROVED:'DEPARTED',DEPARTED:'RETURNED',RETURNED:'COMPLETED'}[leave.status];if(!next)return{ok:false,code:'INVALID_LEAVE_STATE'};const requestKey='LEAVE:'+id+':'+next;if(leave.lastMutationKey===requestKey)return{ok:true,code:'LEAVE_ALREADY_PROCESSED',next};const check=validateLeaveState(d,id,next);if(!check.ok){const correlationId='COR-'+crypto.randomUUID().slice(0,8).toUpperCase();appendAuditToState(d,'LEAVE_'+next+'_BLOCKED','LEAVE',id,'DENIED',correlationId);return{ok:false,code:check.code,correlationId}}const detainee=d.detainees.find(x=>x.id===leave.detaineeId);if(!detainee||detainee.status!=='AKTIF'){const correlationId='COR-'+crypto.randomUUID().slice(0,8).toUpperCase();appendAuditToState(d,'LEAVE_'+next+'_BLOCKED','LEAVE',id,'DENIED',correlationId);return{ok:false,code:'DETAINEE_INACTIVE',correlationId}}const before=leave.status,correlationId=leave.correlationId||'COR-'+crypto.randomUUID().slice(0,8).toUpperCase();leave.status=next;leave.updatedAt=new Date().toISOString();leave.lastMutationKey=requestKey;leave.correlationId=correlationId;d.lastMutation={key:requestKey,action:'LEAVE_TRANSITION',completedAt:new Date().toISOString(),from:before,to:next};appendAuditToState(d,'LEAVE_'+next,'LEAVE',id,'SUCCESS',correlationId);return{ok:true,code:'LEAVE_TRANSITIONED',from:before,to:next,correlationId}}
function createMovementCommand(state,options={}){const d=state||read();const detaineeId=String(options.detaineeId||''),roomId=String(options.roomId||''),occurredAt=new Date(String(options.occurredAt||'')),requestKey=String(options.requestKey||('ROOM_TRANSFER:'+detaineeId+':'+roomId+':'+String(options.occurredAt||'')));if(!detaineeId||!roomId||Number.isNaN(occurredAt.getTime()))return{ok:false,code:'MOVEMENT_INPUT_INVALID'};if((d.movements||[]).some(x=>x.requestKey===requestKey))return{ok:true,code:'MOVEMENT_ALREADY_PROCESSED',requestKey};const det=(d.detainees||[]).find(x=>x.id===detaineeId),target=(d.rooms||[]).find(x=>x.id===roomId),currentPlacement=(d.placements||[]).filter(x=>x.detaineeId===detaineeId).sort((a,b)=>String(b.since||'').localeCompare(String(a.since||'')))[0]||null,fromRoom=currentPlacement?((d.rooms||[]).find(x=>x.id===currentPlacement.roomId)||currentPlacement):null;if(!det||det.status!=='AKTIF')return{ok:false,code:'DETAINEE_INACTIVE'};if(!target||target.status!=='ACTIVE')return{ok:false,code:'ROOM_INACTIVE'};if(fromRoom?.id===target.id)return{ok:false,code:'SAME_ROOM'};if(typeof validateMovementState==='function'){const check=validateMovementState(d,detaineeId,roomId);if(!check.ok)return{ok:false,code:check.code}}const correlationId=String(options.correlationId||'COR-'+crypto.randomUUID().slice(0,8).toUpperCase()),movement={id:options.id||'MOV-'+crypto.randomUUID().slice(0,10).toUpperCase(),detaineeId,detaineeCode:det.code,type:String(options.type||'TRANSFER_KAMAR'),fromRoom:fromRoom?(fromRoom.block+' / '+fromRoom.room):'UNMAPPED',toRoom:target.block+' / '+target.room,fromRoomId:fromRoom?.id||null,toRoomId:target.id,note:String(options.note||''),occurredAt:occurredAt.toISOString(),createdAt:new Date().toISOString(),source:'ROOM_TRANSFER',requestKey,correlationId};const placement=assignPlacementCommand(d,{detaineeId,roomId,since:movement.occurredAt,source:'ROOM_TRANSFER',movementId:movement.id,requestKey:'PLACEMENT:'+detaineeId+':'+roomId+':'+movement.occurredAt,correlationId});if(!placement.ok)return{ok:false,code:placement.code,correlationId};d.movements=Array.isArray(d.movements)?d.movements:[];d.movements.unshift(movement);appendAuditToState(d,'MOVEMENT_CREATE','MOVEMENT',movement.id,'SUCCESS',correlationId);d.lastMutation={key:requestKey,action:'ROOM_TRANSFER',completedAt:new Date().toISOString()};return{ok:true,code:'MOVEMENT_CREATED',movement,placement:placement.placement,correlationId}}
function openMovementForDetainee(detaineeId){const id=String(detaineeId||'').trim();if(!id)return toast('ID deteni tidak tersedia.');const d=read(),det=(d.detainees||[]).find(x=>x.id===id);if(!det){audit('MOVEMENT_CONTEXT_DENIED','DETAINEE',id,'DENIED');return toast('Deteni tidak ditemukan.')}if(det.status!=='AKTIF'){audit('MOVEMENT_CONTEXT_DENIED','DETAINEE',id,'DENIED');return toast('Deteni tidak aktif.')}if(typeof window.p5openMovement==='function')return window.p5openMovement(id);if(typeof window.MTAMovementView?.render==='function'){window.MTAMovementView.render();setTimeout(()=>{const select=document.querySelector('#p9moveForm select[name="detaineeId"]');if(select){select.value=id;select.dispatchEvent(new Event('change',{bubbles:true}))}audit('MOVEMENT_CONTEXT_OPEN','DETAINEE',id)},0);return}audit('MOVEMENT_CONTEXT_DENIED','DETAINEE',id,'DENIED');return toast('Modul Pergerakan belum siap.')}
...
const read=()=>window.MTADeteniStateKernel?window.MTADeteniStateKernel.read():(()=>{try{return JSON.parse(localStorage.getItem('mta-deteni-demo-v2')||'{}')}catch{return {}}})();
const write=d=>window.MTADeteniStateKernel?window.MTADeteniStateKernel.write(d):localStorage.setItem('mta-deteni-demo-v2',JSON.stringify(d));
const audit=(action,type,id,result='SUCCESS')=>{const d=read();if(window.MTADeteniStateKernel)window.MTADeteniStateKernel.audit(d,action,type,id,result);else{d.audit=d.audit||[];d.audit.unshift({id:'AUD-'+crypto.randomUUID().slice(0,8).toUpperCase(),action,resourceType:type,resourceId:id||'',result,occurredAt:new Date().toISOString(),actor:'DEMO-OPERATOR',requestId:'REQ-'+crypto.randomUUID().slice(0,8),correlationId:'COR-'+crypto.randomUUID().slice(0,8),policyVersion:'AUTHZ-1.0'})}write(d)};
...
function installNavIconStyle(){
  ...
  const s=document.createElement('style');s.id='mta-unified-nav-icons';s.textContent='.nav button{display:flex;align-items:center;gap:11px}.mta-nav-icon{width:18px;height:18px;flex:0 0 18px;fill:none;stroke:currentColor;stroke-width:1.9;stroke-linecap:round;stroke-linejoin:round}.mta-nav-label{min-width:0}.nav button.active .mta-nav-icon{stroke-width:2.1}.nav button:hover .mta-nav-icon{transform:translateX(1px)}@media(min-width:1025px){body.mta-desktop-enhanced .nav button::before{content:none!important;display:none!important;width:0!important;min-width:0!important;height:0!important;padding:0!important;margin:0!important;border:0!important;box-shadow:none!important}.mta-desktop-enhanced .nav button[data-icon]::before{content:none!important;display:none!important}}';document.head.appendChild(s);
}
...
function monitor(){
  ...
  return '<section class="hero"><h1>Operational Monitor</h1><p class="sub">Unified operational surface — seluruh KPI diturunkan dari shared synthetic state.</p></section><div class="grid stats"><div class="card"><div class="label">Deteni Aktif</div><div class="value">'+m.activeDetainees+'</div><span class="status">STATE</span></div><div class="card"><div class="label">Kamar Aktif</div><div class="value">'+m.activeRooms+'/'+m.masterRooms+'</div><span class="status">MASTER</span></div><div class="card"><div class="label">Occupancy</div><div class="value">'+m.occupancy+'/'+m.capacity+'</div><span class="status">PLACEMENT</span></div><div class="card"><div class="label">Izin Pending</div><div class="value">'+m.pendingLeaves+'</div><span class="status">WORKFLOW</span></div><div class="card"><div class="label">Audit Events</div><div class="value">'+m.audit+'</div><span class="status">EVIDENCE</span></div></div><div class="grid" style="grid-template-columns:2fr 1fr;margin-top:12px"><section class="card"><h2>Operational State</h2><div class="kpis"><div><div class="label">Pergerakan 24 jam</div><b>'+m.recentMovement+'</b></div><div><div class="label">Kapasitas tersedia</div><b>'+m.availableCapacity+'</b></div><div><div class="label">Denied events</div><b>'+m.denied+'</b></div><div><div class="label">Queue events</div><b>'+m.queue+'</b></div></div></section><section class="card"><h2>Quick Actions</h2><div class="toolbar"><button class="btn primary" onclick="show(\'camera-scan\')">⌾ Scan Kamera</button><button class="btn" onclick="show(\'qr-center\')">▣ QR Center</button><button class="btn" onclick="mtaUnifiedOpenMovement(\'${esc(id)}\')">↔ Pergerakan</button><button class="btn" onclick="show(\'room-ops\')">▥ Room Ops</button><button class="btn" onclick="show(\'reports\')">▤ Laporan</button></div></section></div><div class="card" style="margin-top:12px"><h2>Runtime Integrity</h2><div id="mtaRuntimeHealth"></div></div>'
}
function queue(){
...
function finalIntegrityGate(){
  ...
  const persistenceContract=window.MTADeteniStateKernel?.contractTest?window.MTADeteniStateKernel.contractTest():{ok:false,checks:[{name:'KERNEL_UNAVAILABLE',ok:false}]};checks.push({name:'SINGLE_MUTATION_BOUNDARY',ok:persistenceContract.ok&&typeof window.MTADeteniStateKernel?.write==='function'});
