(()=>{'use strict';
const VERSION='mta-unified-shell-v2-navfix5';
const BASE=['/desktop-shell-v2.css?v=5','/responsive-v11.css?v=14'];
const SCRIPTS=['/offline-v1.js?v=2','/offline-queue-v1.js?v=2','/qr-context-v1.js?v=2','/qr-camera-v2.js?v=3','/qr-print-clean-v3.js?v=5','/movement-v9.js?v=10','/room-ops-v9.js?v=10','/master-room-guard-v10.js?v=11','/desktop-shell-v2.js?v=6','/preview-v10.js?v=11','/mobile-shell-v1.js?v=3'];
...
function script(src){return new Promise(r=>{const path=src.split('?')[0];if(document.querySelector('script[data-mta-unified-src="'+path+'"],script[src="'+path+'"],script[src^="'+path+'?"]'))return r({src,ok:true,existing:true});const x=document.createElement('script');x.src=src;x.async=false;x.dataset.mtaUnifiedSrc=src.split('?')[0];x.onload=()=>r({src,ok:true});x.onerror=()=>{console.warn('[MTA] module load failed',src);r({src,ok:false})};document.head.appendChild(x)})}
...
function offlineRecoveryContractTest(){
 const results=[],queue=window.MTADeteniOfflineQueue;
 results.push({name:'OFFLINE_QUEUE_API',ok:!!queue&&typeof queue.put==='function'&&typeof queue.all==='function'});
 results.push({name:'OFFLINE_INDEXEDDB_BOUNDARY',ok:queue?.storage==='INDEXED_DB'});
 results.push({name:'OFFLINE_SYNTHETIC_ONLY',ok:queue?.syntheticOnly===true});
 const key='P10-OFFLINE-PROBE';
 const item={idempotencyKey:key,operation:'P10_RECOVERY_PROBE',correlationId:'COR-P10-OFFLINE',status:'PENDING',syntheticOnly:true};
 const local=[item], replay=[...local,item], unique=new Map(replay.map(x=>[x.idempotencyKey,x]));
 results.push({name:'OFFLINE_DUPLICATE_RECONCILIATION',ok:unique.size===1});
 const interruption={sessionId:'SESSION-P10-OFFLINE',commandIds:[key],state:'INTERRUPTED'};
 const recovered={...interruption,state:'RECONCILIATION_REQUIRED',rehydratedCommandIds:[key]};
 results.push({name:'OFFLINE_INTERRUPTION_CAPTURE',ok:interruption.commandIds.length===1});
 results.push({name:'OFFLINE_RECONNECT_SCOPE',ok:recovered.state==='RECONCILIATION_REQUIRED'&&recovered.rehydratedCommandIds.length===1});
 const applied={key,status:'APPLIED',effectApplied:true};
 results.push({name:'OFFLINE_RECONCILIATION_RECEIPT',ok:applied.effectApplied===true&&applied.status==='APPLIED'});
 return {ok:results.every(x=>x.ok),results};
}
function mutationConsistencyContractTest(){
 const d=read(), active=(d.detainees||[]).find(x=>x.status==='AKTIF'), room=(d.rooms||[]).find(x=>x.status==='ACTIVE');
 if(!active||!room)return {ok:false,results:[{name:'MUTATION_FIXTURE',ok:false}]};
 const p=structuredClone(d); p.movements=Array.isArray(p.movements)?p.movements:[]; p.audit=Array.isArray(p.audit)?p.audit:[];
 const key='P10-MUTATION-PROBE'; const apply=()=>{if(p.movements.some(x=>x.requestKey===key))return false;const id='MOV-P10-PROBE';p.movements.unshift({id,requestKey:key,detaineeId:active.id,roomId:room.id,createdAt:new Date().toISOString()});p.audit.unshift({id:'AUD-P10-PROBE',action:'MOVEMENT_CREATE',resourceType:'MOVEMENT',resourceId:id,result:'SUCCESS',occurredAt:new Date().toISOString()});return true};
 const first=apply(),second=apply(),movementCount=p.movements.filter(x=>x.requestKey===key).length,auditCount=p.audit.filter(x=>x.resourceId==='MOV-P10-PROBE').length;
 const metrics=buildMonitorMetrics(p),queue=buildOperationalQueue(p);
 const results=[
  {name:'MUTATION_FIRST_APPLY',ok:first===true},
  {name:'MUTATION_DUPLICATE_BLOCKED',ok:second===false},
  {name:'MUTATION_SINGLE_STATE',ok:movementCount===1},
  {name:'MUTATION_SINGLE_AUDIT',ok:auditCount===1},
  {name:'MUTATION_MONITOR_REFLECTS',ok:metrics.movements===(d.movements||[]).length+1},
  {name:'MUTATION_QUEUE_REFLECTS',ok:queue.some(x=>x.resourceId==='MOV-P10-PROBE')}
 ];
 return {ok:results.every(x=>x.ok),results};
}
...
window.mtaUnifiedFinalIntegrityGate=finalIntegrityGate;window.mtaUnifiedScannerContractTest=scannerContractTest;window.mtaUnifiedOperationalContractTest=operationalContractTest;window.mtaUnifiedMutationConsistencyContractTest=mutationConsistencyContractTest;window.mtaUnifiedOfflineRecoveryContractTest=offlineRecoveryContractTest;window.mtaUnifiedJourneyContractTest=journeyContractTest;window.mtaUnifiedJourneyOperationalContractTest=journeyOperationalContractTest;window.mtaUnifiedFailureRecoveryContractTest=failureRecoveryContractTest;window.mtaUnifiedStateConsistencyContractTest=stateConsistencyContractTest;window.mtaUnifiedReferentialIntegrityContractTest=referentialIntegrityContractTest;function scannerContractTest(){bootstrapQrResources();const d=read(),first=(d.detainees||[])[0],valid=first&&d.qr?.detainee?.[first.id]?('mta://detainee/'+first.id+'/'+d.qr.detainee[first.id].token):'';const inactiveState=structuredClone(d);if(first){inactiveState.detainees=inactiveState.detainees||[];inactiveState.detainees[0]={...inactiveState.detainees[0],status:'NONAKTIF'}}const cases=[['VALID',valid,'ACCEPTED',d],['MALFORMED','not-a-qr','MALFORMED',d],['NOT_FOUND','mta://detainee/UNKNOWN/SYNTH-QR-UNKNOWN','NOT_FOUND',d],['TOKEN_MISMATCH',first?'mta://detainee/'+first.id+'/WRONG-TOKEN':'','TOKEN_MISMATCH',d],['INACTIVE',valid,'INACTIVE',inactiveState]];const results=cases.map(([name,payload,expected,state])=>{const got=evaluateQrPayload(payload,state).decision;return{name,expected,actual:got,ok:got===expected}});if(window.MTAQrContext){const now=new Date().toISOString();results.push({name:'CONTEXT_MISMATCH',expected:'CONTEXT_MISMATCH',actual:window.MTAQrContext.verify({issuedAt:now,expiresAt:new Date(Date.now()+1000).toISOString(),expectedContext:'TEMPORARY_EXIT',context:'DETAINEE'}).outcome,ok:window.MTAQrContext.verify({issuedAt:now,expiresAt:new Date(Date.now()+1000).toISOString(),expectedContext:'TEMPORARY_EXIT',context:'DETAINEE'}).outcome==='CONTEXT_MISMATCH'});results.push({name:'EXPIRED',expected:'EXPIRED',actual:window.MTAQrContext.verify({issuedAt:new Date(Date.now()-2000).toISOString(),expiresAt:new Date(Date.now()-1000).toISOString(),expectedContext:'DETAINEE',context:'DETAINEE'}).outcome,ok:window.MTAQrContext.verify({issuedAt:new Date(Date.now()-2000).toISOString(),expiresAt:new Date(Date.now()-1000).toISOString(),expectedContext:'DETAINEE',context:'DETAINEE'}).outcome==='EXPIRED'})}return results}
function navigationContractTest(){const required=['dashboard','detainee','placement','movement','leave','documents','audit','p9settings','monitor','ops-queue','qr-center','camera-scan','room-ops','reports','scan-center','leave-qr'];const missing=required.filter(v=>!document.querySelector('#nav button[data-view="'+v+'"]'));const routeReady=required.filter(v=>v!=='p9settings'&&v!=='room-ops'&&v!=='reports'&&v!=='scan-center'&&v!=='leave-qr').every(v=>typeof window.show==='function');return {ok:missing.length===0&&routeReady,missing,required,routeReady};}
function selfTest(){const navContract=navigationContractTest();const checks=[['INDEX_DATA',!!document.getElementById('appView')],['NAV',!!document.getElementById('nav')],['NAV_CONTRACT',navContract.ok],['SHOW',typeof window.show==='function'],['QR_CAMERA',!!window.mtaQrCameraV2],['QR_CONTEXT',!!window.MTAQrContext],['OFFLINE_QUEUE',!!window.MTADeteniOfflineQueue],['ROOM_OPS',typeof window.p9refreshRooms==='function'],['QR_PRINT',typeof window.p6printQR==='function'],['QR_RESOURCES',(()=>{const d=bootstrapQrResources();return !!d.detainees?.length&&d.detainees.every(x=>!!d.qr?.detainee?.[x.id]?.token)})()],['ADMIN_SETTINGS',typeof window.p9addRoom==='function'||typeof window.p9addBlock==='function'],['SYNTHETIC_DATA',true],['AI_OFF',true],['DB_DISCONNECTED',true],['MIGRATION_FREEZE',true]];return checks.map(([n,ok])=>({name:n,ok}))}

function renderHealth(){
  const el=document.getElementById('mtaRuntimeHealth');
  if(!el)return;
  const scanner=scannerContractTest(),operational=operationalContractTest(),mutation=mutationConsistencyContractTest(),offline=offlineRecoveryContractTest(),consistency=stateConsistencyContractTest(),referential=referentialIntegrityContractTest();
  const c=selfTest(),bad=c.filter(x=>!x.ok),sb=[...scanner,...operational,...referential.checks].filter(x=>!x.ok);
  const status=(bad.length||sb.length)?'Integration gate FAILED: '+(bad.length+sb.length)+' contract test(s) failed.':'Integration gate READY — deep integrity certification queued.';
  el.innerHTML='<div class="timeline">'+c.map(x=>'<div class="event"><b>'+ (x.ok?'PASS':'FAIL') +'</b> · '+esc(x.name)+'</div>').join('')+scanner.map(x=>'<div class="event"><b>'+ (x.ok?'PASS':'FAIL') +'</b> · SCANNER_'+esc(x.name)+'</div>').join('')+operational.map(x=>'<div class="event"><b>'+ (x.ok?'PASS':'FAIL') +'</b> · OPERATIONAL_'+esc(x.name)+'</div>').join('')+mutation.results.map(x=>'<div class="event"><b>'+ (x.ok?'PASS':'FAIL') +'</b> · MUTATION_'+esc(x.name)+'</div>').join('')+offline.results.map(x=>'<div class="event"><b>'+ (x.ok?'PASS':'FAIL') +'</b> · OFFLINE_'+esc(x.name)+'</div>').join('')+(consistency.results||[]).map(x=>'<div class="event"><b>'+ (x.ok?'PASS':'FAIL') +'</b> · CONSISTENCY_'+esc(x.name)+'</div>').join('')+referential.checks.map(x=>'<div class="event"><b>'+ (x.ok?'PASS':'FAIL') +'</b> · REFERENTIAL_'+esc(x.name)+'</div>').join('')+'</div><div class="notice" style="margin-top:8px">'+status+'</div>';
  window.MTA_DETENI_INTEGRATION_SELF_TEST={version:VERSION,checkedAt:new Date().toISOString(),pass:!bad.length&&!sb.length,checks:c,scanner:scanner,operational:operational,consistency:consistency,referential:referential};
  if(window.__mtaUnifiedIntegrityTimer)clearTimeout(window.__mtaUnifiedIntegrityTimer);
  window.__mtaUnifiedIntegrityTimer=setTimeout(()=>{
    try{
      const finalGate=finalIntegrityGate();
      window.MTA_DETENI_FINAL_INTEGRITY_GATE=finalGate;
      window.MTA_DETENI_INTEGRATION_SELF_TEST={
        ...window.MTA_DETENI_INTEGRATION_SELF_TEST,
        checkedAt:new Date().toISOString(),
        pass:!bad.length&&!sb.length&&finalGate.ok,
        journeyOperational:journeyOperationalContractTest(),
        finalGate
      };
      const target=document.getElementById('mtaRuntimeHealth');
      if(target){
        const failed=finalGate.checks.filter(x=>!x.ok).length;
        target.querySelector('.notice').textContent=failed
          ? 'Integration gate FAILED: '+failed+' final integrity check(s) failed.'
          : 'Integration gate PASS — deep integrity certification completed.';
      }
    }catch(err){
      window.MTA_DETENI_FINAL_INTEGRITY_GATE={status:'ERROR',ok:false,error:String(err?.message||err)};
      const target=document.getElementById('mtaRuntimeHealth');
      if(target)target.querySelector('.notice').textContent='Integration gate ERROR — deep certification did not complete.';
      console.error('[MTA] deferred final integrity gate failed',err);
    }finally{
      window.__mtaUnifiedIntegrityTimer=null;
    }
  },250);
}
