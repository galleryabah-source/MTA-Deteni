...
function buildReferentialIntegrityReport(d){
  ...
  placements.forEach(p=>{if(p?.detaineeId&&!detById.has(p.detaineeId))report.placementWithoutDetainee.push({id:p.id,detaineeId:p.detaineeId});const room=roomForPlacement(d,p);if(p?.roomId&&!roomById.has(p.roomId))report.placementWithoutMasterRoom.push({id:p.id,roomId:p.roomId});else if(p?.roomId&&room?.status!=='ACTIVE'&&detById.get(p.detaineeId)?.status==='AKTIF')report.placementWithoutMasterRoom.push({id:p.id,roomId:p.roomId,reason:'ROOM_INACTIVE'})});
