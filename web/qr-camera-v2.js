...
let active=null,ready=null,lastRaw='',lastAt=0;
...
function detected(raw){const value=String(raw||'').trim();if(!value)return;const now=Date.now();if(value===lastRaw&&now-lastAt<1500)return;lastRaw=value;lastAt=now;resolve(value)}
function detected(raw){
  const value=String(raw||'').trim();
  if(!value)return;
  const now=Date.now();
  if(value===lastRaw&&now-lastAt<1500)return;
  lastRaw=value;lastAt=now;
  resolve(value);
}
...
async function start(deviceId){if(!active)active=new Html5Qrcode('mtaQrReader',{verbose:false});const opts={fps:10,qrbox:(w,h)=>{const side=Math.max(210,Math.min(320,Math.floor(Math.min(w,h)*.72)));return {width:side,height:side}},aspectRatio:1.0,disableFlip:false,formatsToSupport:[Html5QrcodeSupportedFormats.QR_CODE]};try{await active.start(deviceId?{deviceId:{exact:deviceId}}:{facingMode:'environment'},opts,(decoded)=>{document.getElementById('mtaQrFrame')?.classList.add('success');status('QR terdeteksi — memproses…');navigator.vibrate?.(40);setTimeout(()=>detected(decoded),160)},()=>{});status('Arahkan kamera ke QR code');const track=document.querySelector('#mtaQrReader video')?.srcObject?.getVideoTracks?.()[0];const caps=track?.getCapabilities?.()||{};const torch=document.getElementById('mtaQrTorch');if(torch){torch.style.display=caps.torch?'inline-block':'none';torch.onclick=async()=>{try{await track.applyConstraints({advanced:[{torch:track.getSettings().torch!==true}]})}catch{status('Flash tidak tersedia pada kamera ini')}}}document.getElementById('mtaQrCamera').onchange=e=>restart(e.target.value);return true}catch(err){console.warn(err);status('Kamera tidak dapat dibuka. Periksa izin kamera dan gunakan HTTPS/LAN secure context.');const hint=document.getElementById('mtaQrHint');if(hint)hint.textContent='Jika kamera tidak tersedia, gunakan tombol Galeri untuk memindai gambar QR.';return false}}
async function restart(id){try{await active?.stop()}catch{}await start(id)}
async function open(){if(document.getElementById('mtaQrScannerModal'))return;ui();status('Menyiapkan mesin scanner…');try{await load();const devices=await cameras();await start(devices.find(d=>/back|rear|environment/i.test(d.label||''))?.id||devices[0]?.id)}catch(err){status('Engine scanner belum tersedia. Gunakan Galeri atau coba lagi.');console.error(err)}}
function inject(){ensureCss();const root=document.querySelector('#appView');if(!root||root.dataset.qrCameraBound)return;root.dataset.qrCameraBound='1';const observer=new MutationObserver(()=>{if(!document.querySelector('#p5scan'))return;if(document.getElementById('mtaCameraScanBtn'))return;const host=document.querySelector('#p5scan .p5c');if(!host)return;const row=host.querySelector('.p5row');if(!row)return;const b=document.createElement('button');b.id='mtaCameraScanBtn';b.className='btn primary';b.type='button';b.textContent='📷 Scan Kamera';b.onclick=open;row.appendChild(b)});observer.observe(root,{childList:true,subtree:true})}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',inject);else inject();window.mtaQrCameraV2={open,close,...(window.__MTA_TEST_MODE__?{simulateDecode:detected}: {})};
})();