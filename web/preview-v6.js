...
function boot(){if(document.querySelector('#mtaPreviewV6'))return;const s=document.createElement('style');s.id='mtaPreviewV6';s.textContent='.p6grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px;margin-top:12px}.p6card{background:#fff;border:1px solid var(--line);border-radius:13px;padding:14px}.p6tag{display:inline-block;padding:4px 8px;border-radius:99px;background:#eef4ff;color:#245ca8;font-size:10px}.p6row{display:flex;gap:8px;align-items:center;flex-wrap:wrap}.p6input{width:100%;padding:10px;border:1px solid var(--line);border-radius:9px}.p6video{width:100%;max-width:520px;border-radius:14px;background:#111;min-height:260px}.p6danger{color:#b42318}.p6ok{color:#16763b}.p6print{display:none}@media(max-width:900px){.p6grid{grid-template-columns:1fr 1fr}}@media(max-width:600px){.p6grid{grid-template-columns:1fr}}';document.head.appendChild(s);const nav=document.querySelector('#nav');[['p6r','Room Ops','p6rooms'],['p6l','Leave QR','p6leaveqr'],['p6c','Camera Scan','p6camera'],['p6d','Reports','p6reports']].forEach(([id,label,v])=>{const b=document.createElement('button');b.id=id; b.dataset.view=v;b.textContent=label;nav.appendChild(b);b.onclick=()=>window.show(v)});const old=window.show;window.show=v=>['p6rooms','p6leaveqr','p6camera','p6reports'].includes(v)?render(v):old(v);ensure()}
...
window.p6qr=(kind,id)=>{const d=ensure(),map=kind==='room'?'room':kind==='leave'?'leave':'detainee',q=d.qr[map][id],x=kind==='room'?d.rooms.find(z=>z.id===id):kind==='leave'?d.leaves.find(z=>z.id===id):d.detainees.find(z=>z.id===id);if(!q||!x)return;const payload='mta://'+kind+'/'+id+'/'+q.token;const label=kind==='room'?x.block+' / '+x.room:kind==='leave'?x.id+' · '+x.destination:x.code+' · '+x.name;openModal(`<div class="dialoghead"><h2>QR ${kind.toUpperCase()}</h2><button class="x" onclick="closeModal()">×</button></div><div class="p6row"><div id="p6qrbox" class="p5qr"></div><div><h3>${E(label)}</h3><span class="p6tag">${E(q.status)}</span><p><code>${E(payload)}</code></p><button class="btn primary" onclick="window.p6printQR('${kind}','${id}')">Cetak QR</button></div></div><div class="notice">Opaque token · QR bukan autentikasi · resolusi tetap melewati policy boundary.</div>`);setTimeout(()=>{if(window.qrcode){const g=qrcode(0,'M');g.addData(payload);g.make();document.querySelector('#p6qrbox').innerHTML=g.createSvgTag(6,0)}},100);audit('QR_VIEW','QR',id)};
window.p6printQR=(kind,id)=>{const d=ensure(),map=kind==='room'?'room':kind==='leave'?'leave':'detainee',q=d.qr[map]?.[id],x=kind==='room'?d.rooms.find(z=>z.id===id):kind==='leave'?d.leaves.find(z=>z.id===id):d.detainees.find(z=>z.id===id);if(!q||!x||!window.qrcode){toast('QR belum siap untuk dicetak');return}const payload='mta://'+kind+'/'+id+'/'+q.token;const g=qrcode(0,'M');g.addData(payload);g.make();const svg=g.createSvgTag(10,0);const w=window.open('','_blank','width=520,height=520');if(!w){toast('Popup diblokir browser. Izinkan popup untuk mencetak QR.');return}w.document.open();w.document.write('<!doctype html><html lang="id"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>QR MTA DETENI</title><style>@page{size:A6 portrait;margin:0}*{box-sizing:border-box}html,body{margin:0;padding:0;background:#fff}body{width:100vw;min-height:100vh;display:flex;align-items:center;justify-content:center;overflow:hidden}.qr{width:78mm;height:78mm;display:flex;align-items:center;justify-content:center}.qr svg{width:78mm!important;height:78mm!important;display:block}</style></head><body><div class="qr">'+svg+'</div><script>window.onload=function(){setTimeout(function(){window.print()},250)};<\\/script></body></html>');w.document.close();audit('QR_PRINT','QR',id)};
