...
/* MTA DETAINER TABLE ALIGNMENT FIX V1 */
(function(){
  'use strict';
  const STYLE_ID='mta-detainee-table-alignment-v1';
  function installDetaineeTableStyle(){
    if(document.getElementById(STYLE_ID)) return;
    const s=document.createElement('style');
    s.id=STYLE_ID;
    s.textContent=`
      .detainee-table th:first-child,.detainee-table td:first-child{
        width:52px!important;min-width:52px!important;max-width:52px!important;
        text-align:center!important;white-space:nowrap!important;
      }
      .detainee-table th:nth-child(2),.detainee-table td:nth-child(2){
        width:150px!important;min-width:150px!important;
        white-space:nowrap!important;text-align:left!important;
      }
      .detainee-table td:nth-child(2) b{
        display:inline-block;white-space:nowrap!important;
        font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,"Liberation Mono",monospace;
        letter-spacing:.01em;
      }
      .detainee-table td:first-child .detainee-row-no{font-weight:700;color:#526176;}
    `;
    document.head.appendChild(s);
  }
  function normalizeDetaineeTable(){
    const table=document.querySelector('.detainee-table');
    if(!table) return;
    installDetaineeTableStyle();
    const head=table.tHead?.rows?.[0];
    if(head && head.cells.length===6){
    ...
    rows.forEach((row)=>{
      if(row.cells.length===6){
        ...
        td.innerHTML='<span class="detainee-row-no"></span>';
    ...
    rows.forEach((row)=>{
      ...
      if(visible){
        no+=1;
        const holder=cell.querySelector('.detainee-row-no');
        if(holder) holder.textContent=String(no);
        else if(row.cells.length===7) cell.textContent=String(no);
      }else{
        const holder=cell.querySelector('.detainee-row-no');
  ...
  function bootDetaineeTableAlignment(){
    installDetaineeTableStyle();
    normalizeDetaineeTable();
    const root=document.getElementById('appView')||document.body;
    const observer=new MutationObserver(()=>normalizeDetaineeTable());
    observer.observe(root,{subtree:true,childList:true,attributes:true,attributeFilter:['style','class']});
    setInterval(normalizeDetaineeTable,1000);
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',bootDetaineeTableAlignment);
  else bootDetaineeTableAlignment();
})();