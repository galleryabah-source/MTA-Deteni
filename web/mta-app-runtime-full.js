...
function buildReportEvidence(r){
  ...
  const relatedAudit=(db.audit||[]).filter(a=>ids.includes(a.resourceId)||a.resourceId===r.documentId);
  const auditIds=relatedAudit.map(a=>a.id).slice(0,200);
  const sourceCorrelationIds=[...new Set(sourceRecords.flatMap(s=>{const owner=[db.detainees,db.movements,db.leaves,db.placements,db.rooms].flat().find(x=>x?.id===s.id);return owner?.correlationId?[owner.correlationId]:[]}).concat(relatedAudit.map(a=>a.correlationId).filter(Boolean)))];
  return {capturedAt:now(),reportCorrelationId:r.correlationId||null,sourceCorrelationIds,sourceRecords,auditIds,sourceRecordCount:sourceRecords.length,auditEventCount:auditIds.length};
}
