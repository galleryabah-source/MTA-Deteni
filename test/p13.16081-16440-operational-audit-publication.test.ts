...
function prepared(fingerprint = "FP-PUB", publicationId = "PUB-P") {
  ...
  const projection = createLocalRuntimeRecoveryOperationalAuditProjection({ projectionId: `PROJ-PUB-${fingerprint}`, evidenceCertification: c.evidenceCertification, evidence: c.evidence });
