...
test("local PostgreSQL adapter feeds the shared resolver",async()=>{
 const db={query:async()=>({rows:[{resource_id:"DET-001",resource_type:"DETAINEE",token_hash:"HASHED",status:"ACTIVE",context:"RUDENIM_STAY",issued_at:"2026-09-22T08:00:00Z",expires_at:null}]})};
 const verifier={hash:async()=> "HASHED"};
...
test("Supabase adapter feeds the shared resolver",async()=>{
 let accessToken="";
 const http={resolve:async(input:any)=>{accessToken=input.accessToken;return {resourceId:"DET-001",resourceType:"DETAINEE",token:"RAW",status:"ACTIVE",context:"RUDENIM_STAY",issuedAt:"2026-09-22T08:00:00Z"}}};
