...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    if(req.method==="POST") return json(req,{ok:true,resource,role,data:p.data,row:p.row,replayed:!!p.replayed},p.replayed?200:201);
    if(req.method==="PATCH") return json(req,{ok:true,resource,role,data:p.row,replayed:!!p.replayed});
    return json(req,{ok:true,resource,role,deleted:{id:p.row?.id},replayed:!!p.replayed});
