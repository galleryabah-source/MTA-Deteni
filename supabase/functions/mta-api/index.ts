...
const json=(req,body,status=200,context={})=>{const requestId=context.requestId||req.headers.get("X-Request-Id")||crypto.randomUUID();const correlationId=context.correlationId||req.headers.get("X-Correlation-Id")||requestId;const response=new Response(JSON.stringify(body),{status,headers:{...cors(req),"X-Request-Id":requestId,"X-Correlation-Id":correlationId}});return response;};
...
Deno.serve(async(req)=>{
  ...
  }catch(error){const safe=sanitizeObservabilityError(error);emit("ERROR","request.failed",{status:500,outcome:"FAILED",errorCode:"UNHANDLED_API_ERROR",durationMs:performance.now()-startedAt});console.error(JSON.stringify({service:"mta-api",event:"request.error",requestId,correlationId,error:safe}));return json(req,{ok:false,error:"UNHANDLED_API_ERROR"},500,{requestId,correlationId});}\n  finally { emit("INFO","request.completed",{status:200,outcome:"SUCCESS",durationMs:performance.now()-startedAt}); }
});
