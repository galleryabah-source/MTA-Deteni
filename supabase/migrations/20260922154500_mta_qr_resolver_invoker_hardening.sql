-- MTA DETENI QR resolver security hardening
-- The resolver is intentionally callable by authenticated clients, but does not need
-- SECURITY DEFINER because the authenticated role now has RLS-protected SELECT
-- access to the QR registry.
...
grant select on table public.mta_qr_registry to authenticated;
