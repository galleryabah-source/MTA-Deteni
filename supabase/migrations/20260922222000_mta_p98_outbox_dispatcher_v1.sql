...
returns setof mta_internal.outbox_events
language sql
set search_path = pg_catalog, mta_internal
as $$
  update mta_internal.outbox_events
  set status='PROCESSING', attempts=attempts+1, locked_at=now()
  where event_id in (
    select event_id from mta_internal.outbox_events
...
as $$
  update mta_internal.outbox_events
  set status='PENDING',
      available_at=now()+make_interval(secs=>greatest(1, least(p_backoff_seconds, 86400))),
      locked_at=null,
      last_error=left(coalesce(p_error,'OUTBOX_DISPATCH_FAILED'),2000)
